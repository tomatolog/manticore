...
-  added `#1860 <http://sphinxsearch.com/bugs/view.php?id=1860>`__,
   `USERVARs <../set_syntax.html>`__ for distributed indexes

-  added `#1689 <http://sphinxsearch.com/bugs/view.php?id=1689>`__,
   `GROUP BY JSON <../select_syntax.html>`__ attributes

-  added `FACET <../select_syntax.html>`__ keyword

-  added Go MySQL connector support

-  added `IDF boost <../extended_query_syntax.html>`__ keyword modifier

-  added `MAYBE <../extended_query_syntax.html>`__ fulltext operator
...
-  removed
   `max\_matches <../sphinx_deprecations_and_changes_in_default_configu.html>`__
   config key
...
-  fixed #1942, crash in `SHOW THREADS <../show_threads_syntax.html>`__
   command
...
-  fixed #1919,
   `TSV <../tsvpipecsvpipe_tabcomma_separated_values_data_sour.html>`__
   bitcount attributes indexation issue

-  fixed #1916, `COUNT(\*) <../select_syntax.html>`__ with empty result
   set

-  fixed #1910, JSON parsing issue

-  fixed #1906, `ZONE <../extended_query_syntax.html>`__ constraints for
   expanded terms

-  fixed #1904, race condition in RT indexes on saving disk chunk

-  fixed #1899, crash on `CALL KEYWORDS <../call_keywords_syntax.html>`__

-  fixed #1893, ``searchd`` crashes on expressions like 'a<<(\*!b)'

-  fixed #1884, crash with `SNIPPET() <../select_syntax.html>`__ function
   over distributed index

-  fixed #1883, crash at expanded keyword with hitless index

-  fixed #1870, crash on `ORDER BY JSON <../select_syntax.html>`__
   attributes
