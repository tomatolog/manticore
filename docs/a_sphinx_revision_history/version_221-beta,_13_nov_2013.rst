...
-  added `ALTER TABLE <../alter_syntax.html>`__ that can add attributes to
   disk and RT indexes on the fly
-  added ATTACH support for non-empty RT target indexes
-  added Chinese segmentation with
   `RLP <../index_configuration_options/morphology.html>`__ (Rosette
   Linguistics platform) support
-  added English, German
   `lemmatization <../index_configuration_options/morphology.html>`__
   support
-  added `HAVING <../select_syntax.html>`__ support to SELECT statement,
   filtering on aggregate values is now possible
-  added `N-best GROUP BY <../select_syntax.html>`__ extension to return
...
-  added
   `CSV <../tsvpipecsvpipe_tabcomma_separated_values_data_sour.html>`__,
...
-  added index
   `type=template <../index_configuration_options/type.html>`__ directive
...
-  added table functions mechanism, and
   `REMOVE\_REPEATS() <../select_syntax.html>`__ table function
-  added support for arbitrary expressions in WHERE for DELETE queries

Ranking related features
~~~~~~~~~~~~~~~~~~~~~~~~

-  added OPTION `local\_df=1 <../select_syntax.html>`__, an option to
   aggregate IDFs over local indexes (shards)
-  added `UDF <../sphinx_udfs_user_defined_functions.html>`__
   XXX\_reinit() method to reload UDFs with ``workers=prefork``
-  added comma-separated syntax to `OPTION <../select_syntax.html>`__
   ``idf``, ``tfidf_unnormalized`` and ``tfidf_normalized`` flags
-  added ``lccs``, ``wlccs``, ``exact_order``, ``min_gaps``, and ``atc``
   `ranking
   factors <../search_results_ranking/field-level_ranking_factors.html>`__
-  added ``sphinx_get_XXX_factors()``, a faster interface to access
   ...
   in UDFs
-  added support for
   `exact\_hit <../search_results_ranking/field-level_ranking_factors.html>`__,
   `exact\_order <../search_results_ranking/field-level_ranking_factors.html>`__
...
-  added `DESCRIBE <../describe_syntax.html>`__ and
   `–dumpheader <../indextool_command_reference.html>`__ support for
   tokencount attributes (generated by index\_field\_lengths=1
   directive)
-  added RT index query profile, percentages, totals to `SHOW
   PROFILE <../show_profile_syntax.html>`__
-  added ``predicted_time``, ``dist_predicted_time``, ``fetched_docs``,
   ``fetched_hits`` counters to `SHOW META <../show_meta_syntax.html>`__
-  added ``total_tokens`` and ``disk_bytes`` counters to `SHOW INDEX
   STATUS <../show_index_status_syntax.html>`__

General features
~~~~~~~~~~~~~~~~

-  added
   `ALL() <../5_searching/expressions,_functions,_and_operators/miscellaneous_functions.html#expr-func-all>`__,
   `ANY() <../5_searching/expressions,_functions,_and_operators/miscellaneous_functions.html#expr-func-any>`__
   and
   `INDEXOF() <../5_searching/expressions,_functions,_and_operators/miscellaneous_functions.html#expr-func-indexof>`__
   functions for JSON subarrays
-  added
   `MIN\_TOP\_WEIGHT() <../5_searching/expressions,_functions,_and_operators/miscellaneous_functions.html#expr-func-min-top-weight>`__,
   `MIN\_TOP\_SORTVAL() <../5_searching/expressions,_functions,_and_operators/miscellaneous_functions.html#expr-func-min-top-sortval>`__
   functions
-  added
   `TOP() <../search_results_ranking/ranking_factor_aggregation_functions.html>`__
   aggregate function to expression ranker
-  added a check for duplicated tail hit positions in `indextool
   –check <../indextool_command_reference.html>`__
-  added
   `compact\_in <../searchd_query_log_formats/sphinxql_log_format.html>`__
   option to
   `query\_log\_format=sphinxql <../searchd_program_configuration_options/querylog_format.html>`__
...
-  added `indexer –nohup <../indexer_command_reference.html>`__ and
   `indextool –rotate <../indextool_command_reference.html>`__ switches to
...
-  added
   `shutdown\_timeout <../searchd_program_configuration_options/shutdowntimeout.html>`__
   directive to ``searchd`` config section
-  added signed values support for
   `INTEGER() <../5_searching/expressions,_functions,_and_operators/type_conversion_functions.html#expr-func-integer>`__
   and
   `UINT() <../5_searching/expressions,_functions,_and_operators/numeric_functions.html#expr-func-uint>`__
   function
-  added snippet generation options to
   `SNIPPET() <../select_syntax.html>`__ function
...
-  optimized SELECT (1.02x to 3.5x speedup, depending on index schema
   size)
-  optimized `UPDATE <../update_syntax.html>`__ (up to 3x faster on big
   updates)
...
-  changed post-morphology
   `wordforms <../index_configuration_options/wordforms.html>`__ behavior,
   now it works as
   ``&#039;if ( stem(token)==stem(abc) ) emit(def)&#039;``
-  changed the `config
   defaults <../sphinx_deprecations_and_changes_in_default_configu.html>`__
...
-  changed the default SphinxAPI matching mode to
   `SPH\_MATCH\_EXTENDED2 <../matching_modes.html>`__
...
-  fixed #1683, under certain conditions
   `stopwords <../index_configuration_options/stopwords.html>`__ were not
...
-  fixed #1549, `OPTION <../select_syntax.html>`__
   ``idf=tfidf_normalized`` was ignored for distributed queries
-  fixed that `ORDER BY RAND() <../select_syntax.html>`__ was not affected
   by ``index_weights``
...
-  fixed `GROUP\_CONCAT <../select_syntax.html>`__ result length limit
   (was implicitly limited by 1024 bytes)
-  fixed agent query distribution in HA mirroring
-  fixed duplicates check for `quorum
   operator <../extended_query_syntax.html>`__, it works ok now for
   expanded keywords
-  fixed off-by-1 query positions of words in indexes with wordforms and
   `blended characters <../extended_query_syntax.html>`__
-  fixed wrong ``lcs`` and
   `min\_best\_span\_pos <../search_results_ranking/field-level_ranking_factors.html>`__
