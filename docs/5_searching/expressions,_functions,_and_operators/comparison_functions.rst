...
-  ``IF()`` behavior is slightly different that that of its MySQL
   ...
   In the f.html case, the comparison operator <> will return 0.0 (false)
...
-  IN(expr,val1,val2,…), introduced in version 0.9.9-rc1, takes 2 or
   ...
   Starting with 0.9.9-rc2, f.html argument can also be a MVA attribute.
   ...
   belongs to the list in the given global user variable. F.html argument
...
-  INTERVAL(expr,point1,point2,point3,…), introduced in version
   ...
   argument that is less than the f.html argument: it returns 0 if
