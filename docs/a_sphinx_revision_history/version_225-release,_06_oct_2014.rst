...
-  added OPTION `rand\_seed <../select_syntax.html>`__ which affects ORDER
   BY RAND()
...
-  fixed #2054, `SHOW THREADS <../show_threads_syntax.html>`__ hangs if
   queries in prefork mode

-  fixed #2057, WARNING at ``indexer`` on duplicated wordforms

-  fixed #2066, snippet generation with
   `weight\_order <../additional_functionality/buildexcerpts.html>`__
   enabled
