...
-  added `FLUSH RAMCHUNK <../flush_ramchunk_syntax.html>`__ statement

-  added `SHOW PLAN <../show_plan_syntax.html>`__ statement

-  added support for `GROUP BY <../select_syntax.html>`__ on multiple
   attributes

-  added
   `BM25F() <../search_results_ranking/expression_based_ranker_sphrank_expr.html>`__
   function to ``SELECT`` expressions (now works with the expression
   based ranker)

-  added `indextool <../indextool_command_reference.html>`__ ``--fold``
...
-  added missing
   `rt\_attr\_bool <../index_configuration_options/rtattr_bool.html>`__
   directive
...
-  optimized `xmlpipe2 <../xmlpipe2_data_source.html>`__ indexing (up to 9
   times faster on some schemas)

Bug fixes
~~~~~~~~~

-  fixed #1684, `COUNT(DISTINCT smth) <../select_syntax.html>`__ with
...
-  fixed #1628, `GROUP\_CONCAT() <../select_syntax.html>`__ and
   `GROUPBY() <../select_syntax.html>`__ support for distributed agents
...
-  fixed #1615, global IDF vs exact term (=term) fixed global IDF for
   ...
   option <../index_configuration_options/globalidf.html>`__
...
-  fixed #1578, `SORT BY <../sorting_modes.html>`__ string attribute via
...
-  fixed #1508,
   ...
   `agent\_connect\_timeout <../searchd_program_configuration_options/agentconnect_timeout.html>`__
   with epoll path
...
-  fixed #1484, `INSERT <../insert_and_replace_syntax.html>`__ into RT vs
   no JSON attribute
...
-  fixed #1460, `aggregate functions <../select_syntax.html>`__ ``AVG()``,
   ``MAX()``, ``MIN()``, ``SUM()`` do not work for JSON attributes

-  fixed #1459, ``BM25F`` doesn't work with
   `field\_string <../data_source_configuration_options/sqlfield_string.html>`__
   fields
...
-  fixed #1446, `WEIGHT() <../search_results_ranking/README.html>`__ did
...
-  fixed #1443,
   `morphology=lemmatizer\_ru\_all <../index_configuration_options/morphology.html>`__
   now works with
   `index\_exact\_words=1 <../index_configuration_options/indexexact_words.html>`__
...
-  fixed #1384, added possibility to define any own DSN line with
   `source=mssql <../data_source_configuration_options/README.html>`__
   (like as in ``source=odbc``)

-  fixed `ATTACH <../attach_index_syntax.html>`__ vs wordforms or
...
-  fixed `max\_predicted\_time <../select_syntax.html>`__ integer
   overflows
...
-  fixed preprocessor definitions for
   `RE2 <../index_configuration_options/regexpfilter.html>`__ in VS
   solution
